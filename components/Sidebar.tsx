    ...
    <aside className="fixed left-0 top-0 h-screen w-[260px] bg-cyber-panel/90 backdrop-blur-xl border-r border-cyber-border flex flex-col z-50">
      ...
      <div className="p-8 pb-4 relative overflow-hidden">
        ...
        <div className="flex items-center gap-3 mb-1">
           ...
           <div>
             <h1 className="font-black text-xl tracking-wider text-cyber-text italic">AERO<span className="text-cyber-cyan">.OS</span></h1>
      ...
      <nav className="flex-1 overflow-y-auto px-4 space-y-2 custom-scrollbar mt-4">
        <div className="text-[10px] font-mono text-cyber-dim px-4 py-2 uppercase tracking-widest border-b border-cyber-border/50 mb-2">
          // 功能模块
        </div>
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-4 px-4 py-3 border-l-2 transition-all duration-300 group relative overflow-hidden ${
                isActive 
                  ? 'bg-cyber-cyan/10 border-cyber-cyan text-cyber-cyan shadow-[inset_10px_0_20px_-10px_rgba(var(--cyan-rgb),0.3)]' 
                  : 'border-transparent text-cyber-dim hover:text-cyber-text hover:bg-cyber-text/5'
              }`}
            >
              <item.icon 
                size={18} 
